...
Ultrasonic localization achieves very high positional accuracy, reported between 0.01 and 0.1 m, due to the low propagation speed of sound, which simplifies time-of-flight measurements. However, this accuracy is limited to very small coverage areas of 2 - 10 m, making ultrasonic systems inherently localized. The paper\cite{bibbo_overview_2022} highlights that ultrasound requires line-of-sight conditions and is strongly affected by obstacles, reflections, and human bodies, all of which are unavoidable in densely populated environments. In highly crowded scenarios, people themselves become dynamic obstacles, continuously blocking and distorting acoustic paths, resulting in unstable localization.

Therefore, Ultrasonic systems perform poorly under scalability metric because they require dense anchor deployment, strict minimum separation distances, and protection zones to avoid signal conflicts. As the number of tags increases, ultrasonic pulses overlap, causing interference and forcing complex scheduling mechanisms that degrade update rates and prevent real-time operation. These constraints make ultrasonic localization impractical for environments where large crowds must be tracked simultaneously.
